...
/***************************code begins here*****************************/
//ssssp dijkstra for directed graph with positive edge

vector<int> dijkstra(vector<pair<int,int> > g[],int n,int u){
	...
	while(!st.empty()){
		...
		for(auto pr:g[v]){
			if(dis[v]+pr.S<dis[pr.F]){
				if(dis[pr.F]!=mymax)st.erase({dis[pr.F],pr.F});
				dis[pr.F]=dis[v]+pr.S;
				parent[pr.F]=v;
				st.insert({dis[pr.F],pr.F});
			}
		}
	}
	return parent;
