...
class featured_tree{
	...
	vector<vector<int> > g;
	int n,h,tim,root;
	vector<int> depth,in,out,parent;
	vector<vector<int> > up;     //up[n][log(n)]  or up[n][h]
	
	void dfs(int u,int par){
		tim++;in[u]=tim;
		if(par!=u)depth[u]=depth[par]+1;
		parent[u]=par;
		up[u][0]=par;
		for(auto v:g[u]){
			if(v==par)continue;
	...
	featured_tree(vector<vector<int> > tg,int tn,int ruut){
		g=tg;
		n=tn;
		tim=0;
		root=ruut;
...
int main(){
	...
	featured_tree tree(g,n,1);
	int q;cin>>q;
	while(q--){
		int u,v;cin>>u>>v;
		cout<<tree.lca(u,v)<<"   "<<tree.lca(v,u)<<' '<<tree.depth[tree.lca(u,v)]<<'\n';
	}
