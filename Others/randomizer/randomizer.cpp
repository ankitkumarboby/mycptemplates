...
//randomiser RNG uniform_int_distribution<long long>(L, R)(rng);
...
//	freopen("input.txt","r",stdin);
	...
	cin>>test;
	while(test--){
		int n=uniform_int_distribution<int>(1, 40000)(rng);
		int m=5;
		set<int> st;
		fo(i,1,m)st.insert(uniform_int_distribution<int>(0, 9)(rng));
		m=st.size();
		cout<<n<<' '<<m<<'\n';
		for(int i:st)cout<<i<<' ';
		cout<<'\n';
